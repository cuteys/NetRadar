...
const linuxUninstallCommand = computed(() => {
  const proxyPrefix = useGhProxy.value ? 'https://gh-proxy.com/' : ''
...
const windowsInstallCommand = computed(() => {
  ...
  const tls = radar.systemSettings?.use_tls ? ' -TLS' : ''
  const proxyPrefix = useGhProxy.value ? 'https://gh-proxy.com/' : ''
  return `& ([scriptblock]::Create((irm ${proxyPrefix}https://raw.githubusercontent.com/cuteys/NetRadar/master/install-agent.ps1))) -Server "${host}" -Token "${token}"${tls}`
})

const windowsUninstallCommand = computed(() => {
  const proxyPrefix = useGhProxy.value ? 'https://gh-proxy.com/' : ''
  return `& ([scriptblock]::Create((irm ${proxyPrefix}https://raw.githubusercontent.com/cuteys/NetRadar/master/install-agent.ps1))) -Uninstall`
})

const currentInstallCommand = computed(() => {
  return installPlatform.value === 'linux' ? linuxInstallCommand.value : windowsInstallCommand.value
})

const currentUninstallCommand = computed(() => {
  return installPlatform.value === 'linux' ? linuxUninstallCommand.value : windowsUninstallCommand.value
})
...
const rawBinaryCommand = computed(() => {
  ...
  const host = radar.systemSettings?.agent_server_addr || defaultHost
  const token = radar.systemSettings?.agent_token || 'netradar_secret_token_12345'
  const binName = installPlatform.value === 'windows' ? '.\\agent.exe' : './agent'
  return `${binName} -server "${proto}//${host}/ws/agent" -token "${token}"`
})

const copyCommand = (text?: string) => {
  const toCopy = typeof text === 'string' ? text : currentInstallCommand.value
        ...
        >
          ...
          <div v-else class="flex-1 overflow-y-auto space-y-4 pr-1">
            ...
            <div class="p-3.5 rounded-2xl bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 space-y-2.5">
              <!-- Top Row: Title & Platform Switcher -->
              <div class="flex items-center justify-between flex-wrap gap-2 text-xs font-medium">
                <span class="flex items-center gap-1.5 text-slate-800 dark:text-slate-100">
                  ...
                  <span>一键自动部署 Agent（开机自启守护）</span>
                </span>
                
                <div class="flex items-center gap-3 flex-wrap">
                  <!-- gh-proxy accelerator checkbox -->
                  ...
                  <!-- Platform Switcher -->
                  <div class="flex items-center p-0.5 rounded-xl bg-slate-200/80 dark:bg-slate-700 border border-slate-300/60 dark:border-slate-600/60 text-[11px]">
                    <button
                      @click="installPlatform = 'linux'"
                      class="px-2.5 py-1 rounded-lg transition-all"
                      :class="installPlatform === 'linux' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-2xs font-semibold' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'"
                    >
                      Linux / OpenWrt / macOS
                    </button>
                    <button
                      ...
                      class="px-2.5 py-1 rounded-lg transition-all"
                      :class="installPlatform === 'windows' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-2xs font-semibold' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'"
                    >
                      Windows PowerShell
                    </button>
                  </div>
              ...
              <div class="relative group">
                ...
                <button
                  @click="copyCommand(currentInstallCommand)"
                  class="absolute top-2 right-2 flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[11px] font-medium transition-all"
                >
              ...
              <div class="flex items-center justify-between flex-wrap gap-2 text-[11px] text-slate-400 pt-0.5">
                <span class="flex items-center gap-1">
                  ...
                  <code class="px-1.5 py-0.5 rounded bg-slate-200/60 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-mono text-[10px]">
                    {{ installPlatform === 'linux' ? '/opt/netradar/agent' : 'C:\\opt\\netradar\\agent' }}
                  </code>
                </span>

                <div class="flex items-center gap-3">
                  <button
                    @click="copyCommand(currentUninstallCommand)"
