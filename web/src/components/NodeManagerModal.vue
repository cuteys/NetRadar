      ...
      >
        ...
        >
          ...
          <div v-if="editingNode" class="flex-1 overflow-y-auto space-y-4 pr-1">
            <div class="flex items-center justify-between gap-3 pb-2 border-b border-slate-200/60 dark:border-slate-800/60">
              <button
                @click="cancelEdit"
                class="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors whitespace-nowrap flex-shrink-0"
              >
            ...
            <div class="p-3.5 rounded-2xl bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 space-y-3">
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">
                    网关定位与 IP
                  </span>
                </div>
                <!-- Mode Switcher: 自动获取 / 手动固定 -->
                <div class="flex items-center p-0.5 rounded-xl bg-slate-200/80 dark:bg-slate-700/80 text-[11px] whitespace-nowrap flex-shrink-0">
                  <button
                    type="button"
            ...
            <div class="p-3.5 rounded-2xl bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 space-y-2.5">
              ...
              <div class="flex items-center justify-between flex-wrap gap-2 text-xs font-medium">
                <span class="flex items-center gap-1.5 text-slate-800 dark:text-slate-100">
                  <Terminal class="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" />
                  <span class="font-semibold">一键部署探针（开机自启与保活）</span>
                </span>
                
                <label class="flex items-center gap-1.5 cursor-pointer text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 text-[11px] select-none whitespace-nowrap">
                  <input
                    type="checkbox"
                    v-model="useGhProxy"
              ...
              <!-- Install Command Code Box with its own dedicated toolbar (never covers text) -->
              <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div class="flex items-center justify-between px-3 py-1.5 bg-slate-50/80 dark:bg-slate-800/60 border-b border-slate-200/50 dark:border-slate-700/50">
                  <span class="text-[10px] text-slate-400 font-mono">Shell 一键部署命令</span>
                  <button
                    @click="copyCommand(installCommand)"
                    class="flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[11px] font-medium transition-all cursor-pointer whitespace-nowrap"
                  >
                ...
                <div class="font-mono text-[11px] text-slate-700 dark:text-slate-300 p-2.5 break-all select-all leading-relaxed overflow-x-auto">
                  {{ installCommand }}
                </div>
              </div>

              <!-- Bottom Row: Install Path & Actions -->
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] text-slate-400 pt-0.5">
                <div class="flex items-center gap-1 min-w-0">
                  <span class="whitespace-nowrap flex-shrink-0">安装目录:</span>
                  <code class="px-1.5 py-0.5 rounded bg-slate-200/60 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-mono text-[10px] truncate">
